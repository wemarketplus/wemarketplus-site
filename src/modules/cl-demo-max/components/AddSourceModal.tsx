  ...
  return (
    <ModalShell title="+ Add Referral Source" subtitle="Source will appear in lead dropdowns, activity notes, and ROI tracking" borderColor="rgba(167,139,250,.35)" maxWidth="sm:max-w-[580px]" saveLabel="💾 Save Referral Source" onSave={save}>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <Field label="Contact Name *" wide><input className={FI} placeholder="Dr. Amanda Chen" value={f.name} onChange={(e) => set('name', e.target.value)} /></Field>
        <Field label="Organization / Facility"><input className={FI} placeholder="Baylor Medical Center" value={f.org} onChange={(e) => set('org', e.target.value)} /></Field>
        <Field label="Title / Role"><input className={FI} placeholder="Physician" value={f.title} onChange={(e) => set('title', e.target.value)} /></Field>
        ...
        <Field label="Email"><input className={FI} type="email" placeholder="dr.chen@baylor.com" value={f.email} onChange={(e) => set('email', e.target.value)} /></Field>
        <Field label="Referral Type"><select className={FI} value={f.type} onChange={(e) => set('type', e.target.value)}>{SOURCE_TYPE_OPTS.map((o) => <option key={o}>{o}</option>)}</select></Field>
        <Field label="Relationship Status"><select className={FI} value={f.status} onChange={(e) => set('status', e.target.value)}>{SOURCE_STATUS_OPTS.map((o) => <option key={o}>{o}</option>)}</select></Field>
        <Field label="Assigned Marketer"><select className={FI} value={f.assigned} onChange={(e) => set('assigned', e.target.value)}>{SOURCE_ASSIGNED_OPTS.map((o) => <option key={o}>{o}</option>)}</select></Field>
        ...
        <Field label="Notes" wide><textarea className={`${FI} min-h-[56px]`} rows={2} placeholder="Relationship history, visit frequency, referral volume…" value={f.notes} onChange={(e) => set('notes', e.target.value)} /></Field>
      </div>
    </ModalShell>
  );
}
