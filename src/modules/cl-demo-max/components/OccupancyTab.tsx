import { APT_STATUS_COLORS, APT_STATUS_LABELS } from '../constants/maxStatus';
import { useMaxDemo } from '../hooks/useMaxDemo';

// Reproduces rOccupancy(): a responsive grid of status-tinted unit cards.
export function OccupancyTab() {
  const { apts, actions } = useMaxDemo();
  return (
    <div className="grid gap-3 [grid-template-columns:repeat(auto-fill,minmax(160px,1fr))]">
      {apts.map((a) => {
        const color = APT_STATUS_COLORS[a.status] || '#8ba4c4';
        return (
          <div
            key={a.id}
            role="button"
          ...
          >
            <div className="mb-2 flex items-center justify-between">
              ...
              <span className="inline-block rounded-full px-2 py-0.5 text-[10px] font-bold" style={{ background: `${color}20`, color }}>
            ...
            {a.rate > 0 && <div className="mt-0.5 text-[11px] text-[#4fc87a]">${a.rate.toLocaleString()}/mo</div>}
          </div>
        );
      })}
    </div>
