...
import { APT_STATUS_LABELS, APT_STATUS_TONE } from '../constants/maxStatus';
import { useMaxDemo } from '../hooks/useMaxDemo';
import { countAptStatus, occupancyRate } from '../utils/maxFormat';

// Reproduces rApartments(): five-stat strip + unit inventory table with
// status-aware row actions and an "+ Add Unit" modal trigger.
export function ApartmentsTab() {
  ...
  return (
    <>
      ...
      <Card title={`All Units (${apts.length})`} action={<DemoButton sm onClick={() => actions.openModal('addUnit')}>+ Add Unit</DemoButton>}>
        <div className="overflow-x-auto">
          <table className={TBL}>
            <thead>
              <tr>{['Unit', 'Type', 'Care', 'Status', 'Resident', 'Rate/Mo', 'MR Tasks', 'Actions'].map((h) => <th key={h} className={TH}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {apts.map((a) => (
                <tr key={a.id} className="hover:bg-white/[0.02]">
                  ...
                  <td className={TD}><Badge tone={APT_STATUS_TONE[a.status] || 'neutral'}>{APT_STATUS_LABELS[a.status] || a.status}</Badge></td>
                  <td className={TD}>{a.resident || <span className="text-[#4b6278]">Vacant</span>}</td>
                  <td className={TD}>{a.rate ? `$${a.rate.toLocaleString()}` : '—'}</td>
                  <td className={TD}>{a.mrTasks > 0 ? <span className="text-[#f87171]">{a.mrTasks} open</span> : <span className="text-[#4fc87a]">Clear</span>}</td>
                  <td className={TD}>
                    <div className="flex gap-[5px]">
                      {a.status === 'on_notice' && <DemoButton sm onClick={() => actions.scheduleMakeReady(a.id)}>Sched MR</DemoButton>}
                      {a.status === 'available' && <DemoButton variant="g" sm onClick={() => actions.setTab('leads')}>Assign Lead</DemoButton>}
