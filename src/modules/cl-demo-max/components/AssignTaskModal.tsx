  ...
  return (
    <ModalShell title="+ Assign Task" subtitle="Task appears in user dashboard, calendar, and task list" borderColor="rgba(167,139,250,.35)" maxWidth="sm:max-w-[540px]" saveLabel="💾 Assign Task" onSave={save}>
      <div className="flex flex-col gap-3">
        <Field label="Task Title *"><input autoComplete="off" className={FI} placeholder="Deep clean Unit 204 — move-out turnover" value={f.task} onChange={(e) => set('task', e.target.value)} /></Field>
        <Field label="Assign To"><select autoComplete="off" className={FI} value={f.assignee} onChange={(e) => set('assignee', e.target.value)}>{ASSIGN_TASK_ASSIGNEE_OPTS.map((o) => <option key={o}>{o}</option>)}</select></Field>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <Field label="Task Type"><select autoComplete="off" className={FI} value={f.type} onChange={(e) => set('type', e.target.value)}>{ASSIGN_TASK_TYPE_OPTS.map((o) => <option key={o}>{o}</option>)}</select></Field>
          <Field label="Priority"><select autoComplete="off" className={FI} value={f.priority} onChange={(e) => set('priority', e.target.value)}>{ASSIGN_TASK_PRIORITY_OPTS.map((o) => <option key={o}>{o}</option>)}</select></Field>
          <Field label="Due Date"><input autoComplete="off" className={FI} type="date" value={f.due} onChange={(e) => set('due', e.target.value)} /></Field>
          <Field label="Related Unit / Lead"><input autoComplete="off" className={FI} placeholder="Unit 204, Dorothy H., etc." value={f.related} onChange={(e) => set('related', e.target.value)} /></Field>
        </div>
        <Field label="Notes"><textarea autoComplete="off" className={`${FI} min-h-[56px]`} rows={2} placeholder="Task details, special instructions…" value={f.notes} onChange={(e) => set('notes', e.target.value)} /></Field>
