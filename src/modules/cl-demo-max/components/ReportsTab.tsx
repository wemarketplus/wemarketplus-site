...
import { REPORT_STATUS_LABEL } from '../constants/maxStatus';
import { useMaxDemo } from '../hooks/useMaxDemo';
import { buildMaxReportCsv } from '../utils/maxCsv';
import { activeLeadCount, countAptStatus, occupancyRate, openHkCount, openMaintCount, openMrCount } from '../utils/maxFormat';

// Reproduces rReports(): KPI strip + operations-summary table with CSV export.
export function ReportsTab() {
  const { apts, leads, mrTickets, maintTickets, hkTasks, actions } = useMaxDemo();
  ...
  return (
    <>
      ...
      <Card title="Operations Summary" action={<DemoButton sm onClick={() => { download(buildMaxReportCsv(apts, leads), 'max-report'); actions.toast('Report exported!'); }}>Export CSV</DemoButton>}>
        <div className="overflow-x-auto">
          <table className={TBL}>
            <thead>
              <tr>{['Category', 'Metric', 'Value', 'Status'].map((h) => <th key={h} className={TH}>{h}</th>)}</tr>
            </thead>
            <tbody>
