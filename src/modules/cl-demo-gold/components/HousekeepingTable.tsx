...
import { HK_STATUS_TONE } from '../constants/goldStatus';
...
export function HousekeepingTable() {
  const { hkTasks, actions } = useGoldDemo();
  ...
  return (
    <div className="overflow-x-auto">
      <table className={TBL}>
        <thead>
          <tr>
            {['Task', 'Unit/Area', 'Type', 'Status', 'Assigned', 'Due', 'Actions'].map((h) => (
        ...
        <tbody>
          {hkTasks.map((t) => (
            <tr key={t.id} className="hover:bg-white/[0.02]">
              ...
              <td className={TD}><Badge tone={HK_STATUS_TONE[t.status]}>{cap(t.status.replace(/_/g, ' '))}</Badge></td>
              <td className={TD}>{t.assignee}</td>
              <td className={TD}>{t.due}</td>
              <td className={TD}>
                <DemoButton variant="x" sm onClick={() => actions.cycleHkStatus(t.id)}>
