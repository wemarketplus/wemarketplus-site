...
import { APT_STATUS_COLORS, APT_STATUS_LABELS, UNIT_STATUS_ORDER } from '../constants/goldStatus';
...
// Reproduces rDash() for the director/admin roles: a six-stat strip plus the
// Unit Status breakdown and Operations Alerts cards.
export function ExecutiveDashboard() {
  const { apts, leads, maintTickets, actions } = useGoldDemo();
  ...
  return (
    <>
      <StatGrid>
        ...
        <StatTile label="Hot Leads" value={hot} valueClassName="text-[#f87171]" sub="Need attention" onClick={() => actions.setTab('leads')} />
        <StatTile label="Open Work Orders" value={openM} valueClassName="text-[#f87171]" sub="Require attention" subClassName="text-[#f87171]" onClick={() => actions.setTab('maintenance')} />
      </StatGrid>

      <div className="grid grid-cols-1 gap-[14px] lg:grid-cols-2">
        <Card
          title="Unit Status"
          action={<DemoButton sm onClick={() => actions.setTab('apartments')}>View All</DemoButton>}
        >
        ...
        <Card title="Operations Alerts">
          <div className="flex flex-col gap-2">
            <div className="rounded-[8px] border border-[#f87171]/15 bg-[#f87171]/[0.06] px-3 py-2.5">
              ...
              <div className="mt-0.5 text-[11px] text-[#6b7fa3]">
                Earl Davis moving out Feb 28.{' '}
                <DemoButton sm onClick={() => actions.scheduleMakeReady(4)}>Schedule Make-Ready</DemoButton>
              </div>
            </div>
            <div className="rounded-[8px] border border-[#f59e0b]/15 bg-[#f59e0b]/[0.06] px-3 py-2.5">
              ...
              <div className="mt-0.5 text-[11px] text-[#6b7fa3]">
                Units 103 and 108 — est. ready Feb 14-15.{' '}
                <DemoButton variant="x" sm onClick={() => actions.setTab('makeready')}>View Board</DemoButton>
              </div>
            </div>
            <div className="rounded-[8px] border border-[#4fc87a]/15 bg-[#4fc87a]/[0.06] px-3 py-2.5">
              ...
              <div className="mt-0.5 text-[11px] text-[#6b7fa3]">
                Dorothy Harrison and Gloria Tran need action today.{' '}
                <DemoButton variant="g" sm onClick={() => actions.setTab('leads')}>View Leads</DemoButton>
              </div>
            </div>
          </div>
        </Card>
