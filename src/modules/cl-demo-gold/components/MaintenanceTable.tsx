...
import { MAINT_PRIORITY_TONE, MAINT_STATUS_TONE } from '../constants/goldStatus';
  ...
  return (
    <div className="overflow-x-auto">
      <table className={TBL}>
        <thead>
          <tr>
            {['Ticket', 'Unit', 'Issue', 'Priority', 'Status', 'Assigned', 'Actions'].map((h) => (
        ...
        <tbody>
          {maintTickets.map((t) => (
            <tr key={t.id} className="hover:bg-white/[0.02]">
              ...
              <td className={TD}><Badge tone={MAINT_PRIORITY_TONE[t.priority]}>{t.priority}</Badge></td>
              <td className={TD}><Badge tone={MAINT_STATUS_TONE[t.status]}>{cap(t.status.replace(/_/g, ' '))}</Badge></td>
              <td className={TD}>{t.assignee}</td>
              <td className={TD}>
                <DemoButton variant="x" sm onClick={() => actions.cycleMaintStatus(t.id)}>
                  {t.status === 'completed' ? 'Reopen' : 'Update'}
                </DemoButton>
              </td>
            </tr>
          ))}
