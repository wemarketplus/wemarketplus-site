  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4 sm:p-8">
      ...
      <div
        ...
        aria-modal="true"
        className={cn(
          'animate-slide-up relative z-10 w-full rounded-[18px] border border-white/[0.1] bg-surface shadow-2xl',
          width,
        )}
      >
        <header className="flex items-center justify-between border-b border-white/[0.07] px-6 py-4">
          ...
          <button
            ...
            aria-label="Close"
            onClick={onClose}
            className="flex h-7 w-7 items-center justify-center rounded-full text-muted hover:bg-white/[0.06] hover:text-foreground"
          >
            <X className="h-4 w-4" />
          </button>
        </header>
        ...
        {footer && (
          <footer className="flex items-center justify-end gap-2 border-t border-white/[0.07] px-6 py-4">
            {footer}
          </footer>
        )}
      </div>
